...
namespace SuperPOS.UI.TA
{
    public partial class FrmTaPaymentDelivery : XtraForm
    {
        ...
        private void btnDriver_Click(object sender, EventArgs e)
        {
            ...
            var lstDriver = CommonData.TaDriver.Where(s => s.DriverWorkDay.Contains(DateTime.Now.DayOfWeek.ToString()));

            foreach (var taDriverInfo in lstDriver)
            {
                if (taDriverInfo.DriverName.Equals(btn.Text))
                {
                    iDriverID = taDriverInfo.ID;
                    btn.Appearance.BackColor = btn.Appearance.BackColor == Color.NavajoWhite
                        ? Color.LightGreen
                        : Color.NavajoWhite;
                }
                else
                    btn.Appearance.BackColor = Color.NavajoWhite;
            }
        }
